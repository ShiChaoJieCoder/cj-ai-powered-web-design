...
You are about to reverse-engineer and rebuild **the target URL(s) provided by the user** as pixel-perfect replicas inside this Next.js codebase.
...
2. Parse `the target URL(s) provided by the user` as one or more URLs. Validate each — if invalid, ask the user. Navigate to each and confirm it loads.
...
8. **Verify asset completeness** — after downloading, check that every `<img>` src and `background-image` URL has a corresponding local file. Re-download any missing items.
